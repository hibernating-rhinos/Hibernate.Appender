//-----------------------------------------------------------------------
// <copyright file="Log4jSpyLogDelegator.java" company="Hibernating Rhinos LTD">
...
/**
 * Delegates JDBC spy logging events to Log4j
 */
public class Log4jSpyLogDelegator implements SpyLogDelegator {
    ...
    public Log4jSpyLogDelegator() {
        loggers.add(jdbcLogger);
        loggers.add(resultSetLogger);
        loggers.add(sqlOnlyLogger);
        loggers.add(sqlTimingLogger);
        loggers.add(connectionLogger);
    }
    ...
    public void sqlTimingOccured(Spy spy, long execTime, String methodCall, String sql) {
        if (IsErrorEnabled(sqlTimingLogger) && (!DriverSpy.DumpSqlFilteringOn || shouldSqlBeLogged(sql))) {
            ...
            } else if (IsWarnEnabled(sqlTimingLogger)) {
    ...
    private boolean IsErrorEnabled(Logger logger) {
        return logger.isEnabledFor(Priority.ERROR);
    }

    private boolean IsWarnEnabled(Logger logger) {
