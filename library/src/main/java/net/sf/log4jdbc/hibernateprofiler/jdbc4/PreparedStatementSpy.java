//-----------------------------------------------------------------------
// <copyright file="PreparedStatementSpy.java" company="Hibernating Rhinos LTD">
...
public class PreparedStatementSpy extends StatementSpy implements PreparedStatement {
    ...
    public String adoNetStyleDumpedSql() {
        StringBuffer dumpSql = new StringBuffer();
        int lastPos = 0;
        int Qpos = sql.indexOf('?', lastPos);  // find position of first question mark
        int argIdx = 0;
        ...
        while (Qpos != -1) {
            ...
            lastPos = Qpos + 1;
            Qpos = sql.indexOf('?', lastPos);
            dumpSql.append("@p").append(argIdx - 1);
        }
        if (lastPos < sql.length()) {
            dumpSql.append(sql.substring(lastPos, sql.length()));  // dump last segment
        }

        for (int i = 0; i < argTrace.size(); i++) {
            dumpSql.append(i == 0 ? ";" : ",");
            dumpSql.append(" @p").append(i).append(" = ").append(argTrace.get(i));
        }

        return dumpSql.toString().trim();
    }
    ...
    public void addBatch() throws SQLException {
        ...
        currentBatch.add(adoNetStyleDumpedSql());
        try {
    ...
    public List<String> cloneCurrentArguments() {
        synchronized (argTrace) {
            List<String> args = new ArrayList<String>();
            for (Object o : argTrace) {
                args.add(o.toString());
            }
            return args;
        }
